...
> The top-level `/requirement` session is the ONLY actor with the `Agent` tool. Spawned agents have no `Agent` tool (a subagent cannot spawn a subagent). The orchestrator drives every stage: spawn → read the returned `HANDOFF` block + re-read `state/active.json` (source of truth) → route per `pipeline.yaml` → repeat, until the Stakeholder gate (Stage 7) or a terminal state.
>
> The model-tier names referenced below (Haiku/Sonnet/Opus) are illustrative bindings of the generic mechanical/standard/deep tiers in `pipeline.yaml §model_tiers`; a product may bind them to whatever its `STACK.md` declares.
>
...
```
On a clean Stakeholder-gate/terminal, `clear` it. `/resume` rebuilds the *scheduler* from this cursor — not just the requirement status — so it re-awaits the exact outstanding spawns instead of guessing.
...
1. **Log** — call:
   ...
   ```
   Pull the usage from the Agent result's `usage` object; omit any field the harness doesn't surface (never fabricate). For personas, log the model the orchestrator actually used (the mechanical or standard tier), not the generator default.
...
## Terminal narration (the Stakeholder is watching)
...
1. **Intake (S1)** — first run the **deterministic lane scan**, then spawn `cto-advisor` (model `intake_judgment`, the standard tier) with the result:
   ```sh
   ...
   - `CHALLENGE-BACK` / `KILL` → STOP, surface to the Stakeholder.
   - `needs_personas` non-empty → spawn each persona via `dynamic-persona-generator` **IN PARALLEL** (one message, multiple Agent calls), each at its tagged depth (`:haiku`→`persona_bounded` = mechanical tier, `:sonnet`→`persona_deep` = standard tier). Then **re-spawn `cto-advisor`** to synthesize; re-read.
   - `ADVANCE` → `express`: next = the one builder (S3); `standard`/`high_stakes`: next = `architect` (S2).
2. **Architect (S2)** → on ADVANCE spawn the tagged builder(s). Multiple tracks (`@backend-developer`/`@frontend-web-developer`/`@mobile-developer`/`@intelligence-engineer`) → spawn **IN PARALLEL**.
3. **Builders (S3) return → POST-BUILD LANE RECHECK FIRST (non-LLM; fixes the only war-game NO).** The lane was fixed from the requirement TEXT at intake, before any code existed. Before routing to review, re-classify on the ACTUAL staged diff:
   ...
   ```
   - `escalate: true` (the diff touched a surface the text missed) → **the express/standard shortcut is VOID.** Restart as `high_stakes`: reinstate architect (if skipped) + security + final; record the escalation + the new surfaces on state; re-run from the architect/build review with full rigor. A "rename a label" that became a `requireRole`/tenant-key change can NO LONGER ship through express with no security review.
...
4. **Reconcile reviews** (use `docs/finding-severity-rubric.md` so Security/QA converge, not bounce each other):
   - both `PASS` → `express` → **run the VETO gate before the Stakeholder gate** (below); else → `final` (S6).
   - any `BOUNCE`/`FAIL` → re-spawn the responsible **builder** with the findings, then **DELTA RE-REVIEW** (next section).
5. **Final (S6) — `final-reviewer`** (model `final_judgment`). `PASS` → **run the VETO gate**, then ensure `awaiting-stakeholder` → STOP at Stakeholder gate. `BOUNCE` → spawn `bounce_target`, continue.

   **VETO gate (non-LLM, before ANY Stakeholder-gate transition):** a VETO must be a state-machine invariant, not your good behavior. Before setting `awaiting-stakeholder`, run:
   ```sh
   uv run ${CLAUDE_PLUGIN_ROOT}/tools/gate_check.py --run-dir <run-folder> --to stakeholder_gate
   ```
   Exit 2 = an unresolved CRITICAL/HIGH or a non-PASS review exists → do NOT advance; route back to the responsible stage. You cannot reach the Stakeholder gate past this check.
6. **Safety bound (lane-aware, pause-and-confirm):** cap at `pipeline.yaml.safety_bound.max_spawns_by_lane` (express 8 / standard 14 / high_stakes 28). On exceed, **PAUSE** and surface to the Stakeholder **with the spawn history** (from the orchestrator cursor: stage, outstanding, spawns, last_route) so loop-vs-legit is distinguishable — the Stakeholder confirms a one-time bump (`/resume`) or kills it. Never assume "loop"; never hard-stop without showing the history. Leave state + cursor consistent for `/resume`.

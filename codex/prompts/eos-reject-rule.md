You are operating the **Engineering OS** under Codex. Read `AGENTS.md` at the repo root (the cross-runtime operating contract), then read `skills/reject-rule/SKILL.md` and execute it for `$ARGUMENTS`.

> Mirrors the `reject-rule` command-skill. Stakeholder-only — reject a proposed rule. Marks the proposal as rejected with a written reason; does NOT delete the proposal (audit trail).

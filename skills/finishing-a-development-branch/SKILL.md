...
Operational depth behind the **Commit discipline durable rule** and the Platform/SRE Stage 8. It exists because a careless git op can wipe real work.

## The Iron Laws

```
1. AGENTS DO NOT COMMIT PRODUCT CODE. You stage it; the Stakeholder commits it.
...
|---|---|---|
| Product code (outside `.engineering-os/`) | **Stakeholder** | The Stakeholder owns what ships; agents propose, the human decides. |
| `.engineering-os/` audit trail (run folder, journals, decision log, state) | **Agent (Platform/SRE)** as `chore(eos):` | The record of what the team did; must flow automatically or `/status` goes stale. |

Two commits, never one mixed commit (e.g. `feat(...)` for product, `chore(eos)` for audit trail — separate).
...
1. STAGE (product) — explicit paths only:
   ...
   uv run ${CLAUDE_PLUGIN_ROOT}/tools/secret_scan.py --staged   # O1 GATE on PRODUCT code too — the Stakeholder commits this later, so a secret must be caught HERE. Exit 1 → unstage, redact, retry. NEVER hand off past this.
...
3. WRITE the deployment report (STAGE-ONLY mode): list staged files; propose commit
   message(s) for the Stakeholder; document the reversibility recipe.
...
5. HAND OFF to the Stakeholder: status `awaiting-stakeholder-commit`, current_owner `stakeholder`.
6. AFTER the Stakeholder commits + pushes:
...
- **Roll back a deploy:** the platform's rollback to the previous known-good revision (per `STACK.md`).
...
`shipped` is a claim that the code is on the remote — you can't claim it from a local commit. Run `git push` (Stakeholder) then `git push --dry-run` (verify "up to date"). If the push 403s or fails, the state is `awaiting-push-fix`, not `shipped` — surface the exact failure so the Stakeholder can fix access.
...
- About to commit product code as an agent — you stage; the Stakeholder commits.
...
| "One commit is simpler than two" | Mixing product + audit trail makes the Stakeholder review the wrong diff. |

## Wiring

| Concern | Owner | Reference |
|---|---|---|
| Stage 8 finishing sequence | **Platform/SRE** | the deploy operating loop |
...
| Deploy rollback | Platform/SRE | `operational-readiness` |

Related: `verification-before-completion`, `subagent-orchestration`, `operational-readiness`.

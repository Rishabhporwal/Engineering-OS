...
3. **Per-service ArgoCD App syncs only the changed one.** CI bumps the image tag in the GitOps manifest for **only** the affected services → ArgoCD's diff is non-empty for **only** those apps → only they sync/canary. `core` changing never touches the `analytics` app's manifest, so `analytics` does not redeploy. **Production-grade ArgoCD config (do these — they're what makes the per-service skip actually work at scale):** (a) each Application sets **`argocd.argoproj.io/manifest-generate-paths`** to its own `infra/k8s/<service>/` path so ArgoCD skips reconciliation + cache-invalidation for apps whose paths a commit didn't touch; (b) generate the Apps with an **ApplicationSet** (git-directory generator) so a new service auto-registers — no hand-wired Application; (c) drive syncs by **webhook, not polling** (push from GitHub → ArgoCD) to cut reconcile lag; (d) Kustomize overlays for `staging`/`production`, external-secrets (never secrets in git), no branch-per-environment.

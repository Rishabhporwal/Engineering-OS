...
## Grafana LGTM binding (a 2026 reference stack)
A common self-hosted binding of the five surfaces: **Mimir** (metrics) · **Loki** (logs) · **Tempo** (traces) · **Grafana** (dashboards), unified by **OpenTelemetry**. The patterns above transfer; the LGTM specifics that matter:
- **The OTel Collector (or Grafana Alloy) is THE single ingestion path** — apps → OTLP → one collector tier fans out to all three backends. **Grafana Agent and Promtail are EOL (Nov 2025) → migrate to Alloy.** Run **agents (DaemonSet) → a stateless gateway pool** that does `tail_sampling` + fan-out (`memory_limiter` first, `batch` last). OTLP-native ingest into Mimir/Loki/Tempo is the 2026 default, not remote-write/Loki-push/Jaeger.
- **The Loki cardinality rule (the #1 outage):** labels = **low-cardinality stream identity only** (cluster/namespace/app/env/level; ~15-label default limit). **High-cardinality values (`trace_id`, `request_id`, tenant/user/order IDs, pod IP, URL) go in structured metadata (Loki 3.x) or line filters — NEVER labels.** A dynamic high-cardinality label explodes streams and melts ingestion. Audit with `logcli series --analyze-labels`.
- **Mimir is for a metrics *platform*, not a default.** Reach for it only at scale (toward 100M–1B+ active series), hard multi-tenancy, long object-store retention, or HA-with-dedup; otherwise plain **Prometheus** (or Prometheus + **Thanos** for cheap long-term) is simpler. Mimir 3.0 (Nov 2025) added a Kafka read/write split + the streaming MQE (~92% less query memory) + native OTLP.
- **Correlation is the whole point** (or you bought three databases): **metric→trace** via exemplars (enable exemplar storage; Grafana exemplar→Tempo link); **trace↔logs** via `trace_id` (Loki derived field → Tempo; Tempo trace-to-logs → Loki) — both directions wired. Tempo 3.0's metrics-generator emits RED span-metrics + service graphs to Mimir and powers exemplars. Object storage (S3) is the substrate for all three — budget LIST/GET request cost + lifecycle up front.

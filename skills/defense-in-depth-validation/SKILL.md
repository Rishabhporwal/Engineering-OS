...
description: Validate at every layer data passes through — entry boundary, business logic, environment guard, audit log. Use after fixing a data-shaped bug, when designing a new multi-tenant endpoint that touches workspace_id or PII, when wiring a new MCP write tool, or any time "we trust the upstream caller" is the only defence.
---

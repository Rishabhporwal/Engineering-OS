...
## AWS binding: Glue Catalog + medallion + ingestion + governance
A common AWS binding (S3 + Iceberg + **AWS Glue Data Catalog**) and the medallion layering:
- **Bronze → Silver → Gold.** **Bronze** = raw, immutable, append-only Iceberg (the SSOT; ingested, not transformed). **Silver** = cleaned/conformed. **Gold** = business marts. dbt does the T (`data-transformation-dbt`); the serving copy is `starrocks-olap` (`Iceberg → dbt → StarRocks → API`, never back). A **phased** path is fine: Phase 1 keeps Silver/Gold warehouse-native for sub-second serving; Phase 3 migrates Silver/Gold *into* Iceberg + adds Athena/Trino/Spark for open multi-engine marts.
- **Bronze ingestion without hand-rolled writers:** materialize Bronze from the broker — **Redpanda Iceberg Topics** (`redpanda-apicurio-avro`) or a managed sink (Tableflow / Iceberg Sink Connector). It's **at-least-once into the table** — carry a stable `event_id = (tenant_id, source, source_event_id)` and **dedup at the Silver `MERGE INTO`** (don't assume exactly-once Bronze). On **Glue**, set an explicit partition spec (the default yields an *unpartitioned* table).
- **Catalog + governance:** the **Glue Iceberg REST** endpoint (sigv4) is the multi-engine catalog; govern row/column access at the catalog with **AWS Lake Formation** FGAC so one policy set holds across Spark/Trino/Athena/StarRocks. Region-pin the warehouse bucket per tenant (`region-and-locale`, `multi-tenancy-isolation`).
- **Maintenance is catalog/engine-managed** with a REST catalog (compaction, snapshot expiry) — a benefit over the object-storage-only catalog; still schedule + monitor it (`pipeline-orchestration`).

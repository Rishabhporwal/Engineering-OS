...
> (a) confirms the mechanics fire, (b) confirms the ~46% cost cut binds live, and (c) settles
...
```
/requirement Update the dashboard empty-state copy on the P&L page to "No data yet for this period."
```
Watch for: lane classified (deterministic), Sonnet builder, a usage row per spawn
(`spawn-heartbeat.jsonl` written by the hook), `/approve` shows the **decision card**.

## 3. Confirm the cost cut bound — the headroom KPI
```sh
uv run tools/dashboard.py    # (CLAUDE_PROJECT_DIR=<brain repo>); open .engineering-os/dashboard.html
```
- **"v2-tier headroom" KPI** on the Tokens tab should now read **~0%** for the new run
  (everything already logged at the right tier). If it still shows ~46%, an agent got
  pinned back to Opus — check that spawn's `model` in `usage.jsonl`.
...
**Put a real Brain feature through the team and watch what breaks.** The OS that's worth
having was built by reacting to real-run observations (O1–O14), not by pre-building features.

...
SQL (paradigm 1, ~$0) ≫ ML (2, <$0.001) ≫ small LLM/Haiku (3, ~$0.005) ≫ frontier/Sonnet (4, ~$0.05–0.50). **Cost ratio ≈ 1 : 100 : 1,000 : 10,000.** Every endpoint/agent declares `@paradigm`; PR is **blocked** if a cheaper paradigm would suffice. Three enforcement layers: default routing, per-feature token budget (soft 80% / hard 100% → degrade), per-workspace monthly cap (soft 70% throttle non-critical / hard 100% critical-path only: Morning Brief, NL query, ticket resolution). Target mix: **85% SQL · 12% ML · 2.5% Haiku · 0.5% Sonnet**.

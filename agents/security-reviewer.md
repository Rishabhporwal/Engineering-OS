...
12. **Classify each finding's timing — must-fix-now vs defer — per the shared rubric [`docs/finding-severity-rubric.md`](../docs/finding-severity-rubric.md)** (conservative tie-break: any doubt → must-fix-now). QA applies the **same** rubric, so the two gates converge instead of diverging on the same finding (the O7 bounce class). Then decide: PASS → Tanvi (Stage 5) | BOUNCE → responsible dev (Vikram/Ananya/Karan/Maya).

...
10. DECIDE the personas (0, 1, or 2) — but do **NOT** spawn them. You are a subagent with no Agent tool. Record the chosen persona type(s) in `02-cto-advisor-review.md`, and RETURN them in your HANDOFF `needs_personas` list (step 16). **Tag each persona's depth (cost lever, 2026-05-24):** append `:haiku` for a **bounded** stress-test (a checklist / toolchain / naming / single-rule angle — Haiku handles these fine and is ~6× cheaper) or `:sonnet` for a **reasoning-heavy** one (migration method, compliance trade-offs, numeric parity, multi-step adversarial). e.g. `needs_personas: ["monorepo-toolchain-realist:haiku", "india-data-isolation-compliance-officer:sonnet"]`. Pick the **cheapest tier that will still surface a real concern** — most single-dimension personas are bounded (`:haiku`); reserve `:sonnet` for genuine reasoning depth. The orchestrator spawns each at the tagged model. The **top-level orchestrator** spawns them in parallel (writing `0N-persona-*.md`) and then re-invokes you to synthesize. For count=0, skip the persona round-trip entirely.

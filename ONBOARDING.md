...
- **Real-browser + visual QA.** Web changes get `/qa-browser` (walks real flows, catches console/network errors, generates a Playwright regression spec) and `/design-review` (0–10 scored visual audit) — the Morning Brief + KPI surfaces have the highest bar.
